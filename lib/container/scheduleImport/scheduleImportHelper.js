	...
	async function getScheduledImports() {
		try {
			...
			const response = await apiFetch( {
				path: 're-esp-campaign-monitor/v1/get-scheduled-imports',
			} );
			const rowData = [];
			response.map( ( item ) => {
				...
				rowData.push( [
					item.id,
					recurrency,
					item.params[ 0 ].credentials.client_id,
				] );
			} );
			setState( {
				...state,
	...
	async function deleteScheduleHandler() {
		...
		if ( selectedRowInfo ) {
			try {
				...
				const response = await apiFetch( {
					path: `re-esp-campaign-monitor/v1/delete-scheduled-import/?id=${ selectedRowInfo.id }`,
					method: 'DELETE',
				} );
